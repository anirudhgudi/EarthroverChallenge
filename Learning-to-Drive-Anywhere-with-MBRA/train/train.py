...
def main(config):
                ...
                if config["project_name"] == "frodobot-gnm":                    
                    ...
                    if data_split_type == "train":
                        if config["model_type"] == "LogoNav":        
                            dataset = FrodbotDataset_LogoNav(repo_id=config["repo_id"], root=config["root"], image_size=config["image_size"], split="train", goal_horizon=config["horizon_short"], goal_horizon2=config["horizon_long"], sacson=config["SACSoN"], context_spacing=3, action_spacing=3)                                          
                        else:
                            dataset = FrodbotDataset_MBRA(repo_id=config["repo_id"], root=config["root"], image_size=config["image_size"], split="train", goal_horizon=config["horizon_short"], sacson=config["SACSoN"], context_spacing=3, action_spacing=1)                                                                                        
                        num_eps = len(dataset.episode_data_index["from"])
                        split_train_test = max(1, int(num_eps * ratio_f))
                        ...
                        num_eps = len(dataset.episode_data_index["from"])
                        split_train_test = max(1, int(num_eps * ratio_f))
                        max_test_ep = max(split_train_test, num_eps - 1)
                        episode_sampler_test = EpisodeSampler_MBRA(dataset, split_train_test, max_test_ep, goal_horizon=config["horizon_short"], data_split_type=data_split_type)                                               
                else:                
    ...
    if config["project_name"] == "frodobot-gnm":
        train_batch = batch_frodobot if train_dataset_sub is not None else config["batch_size"]
        ...
        if train_dataset_sub is not None:
    ...
    if config["model_type"] != "annotate":
        ...
        if "load_exaug" in config:
            ...
            latest_checkpoint_exaug = torch.load(latest_path_exaug, weights_only=False) 
