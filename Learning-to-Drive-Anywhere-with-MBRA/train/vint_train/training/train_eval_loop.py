...
def train_eval_loop_nomad(
    ...
    """
    latest_path = os.path.join(project_folder, f"latest.pth")
    ema_model = EMAModel(parameters=model.parameters(),power=0.75) #[TODO] for diffusion model
    #ema_model = model
    no_emamodel = True
    ...
    for epoch in range(current_epoch, current_epoch + epochs):
        if train_model:
            ...
            train_LogoNav(  
                ...
                latest_path=latest_path,
                dataloader=train_loader,
                dataloader_sub=train_loader_sub if train_loader_sub is not None else train_loader,                
                transform=transform,
                device=device,
                project_folder=project_folder,
                epoch=epoch,
...
def train_eval_loop_lelan_col(
    ...
    latest_path = os.path.join(project_folder, f"latest.pth")
    ema_model = EMAModel(parameters=model.parameters(),power=0.75) #[TODO] for diffusion model
    ema_model_nomad = EMAModel(parameters=model_nomad.parameters(),power=0.75) #[TODO] for diffusion model
