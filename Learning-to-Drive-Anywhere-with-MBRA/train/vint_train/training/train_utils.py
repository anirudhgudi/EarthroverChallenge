...
def train_MBRA(
    ...
    if dataloader_sub is not None:
        dataloader_sub_iter = iter(dataloader_sub)
    else:
        def dummy_iter():
            while True:
                yield (
                    torch.empty((0, 18, 96, 96)),
                    torch.empty((0, 3, 96, 96)),
                    torch.empty((0, 8, 2)),
                    torch.empty((0,)),
                    torch.empty((0, 2)),
                    torch.empty((0,)),
                    torch.empty((0, 8)),
                    torch.empty((0, 3, 128, 416)),
                    torch.empty((0, 2)),
                    torch.empty((0, 3, 3)),
                    torch.empty((0,)),
                )
        dataloader_sub_iter = dummy_iter()
...
def train_LogoNav(
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            try:
                sub_data = next(dataloader_sub_iter)
            except StopIteration:
                dataloader_sub_iter = iter(dataloader_sub) 
                sub_data = next(dataloader_sub_iter)        
            if len(sub_data) >= 13:
                obs_image_sub = sub_data[0]
                goal_image_sub = sub_data[1]
                action_label_sub = sub_data[4]
                dist_label_sub = sub_data[5]
                goal_pos_sub = sub_data[6]
                goal_yaw_sub = sub_data[8]
                dataset_index_sub = sub_data[9]
                action_mask_sub = sub_data[12]
            else:
                (
                    obs_image_sub,
                    goal_image_sub,
            ...
            cos_yaw = torch.cos(goal_yaw_sub)
            sin_yaw = torch.sin(goal_yaw_sub)
            g_pos_flat = goal_pos_sub.view(goal_pos_sub.size(0), -1)
            c_yaw_flat = cos_yaw.view(cos_yaw.size(0), -1)
            s_yaw_flat = sin_yaw.view(sin_yaw.size(0), -1)
            goal_pose_gps_sub = torch.cat((g_pos_flat, c_yaw_flat, s_yaw_flat), dim=1)
            ...
            if actions.ndim > 3:
                actions = actions.view(actions.size(0), 8, 2)
            if action_label_sub.ndim > 3:
                action_label_sub = action_label_sub.view(action_label_sub.size(0), 8, 2)
            if distance.ndim == 2:
                distance = distance.squeeze(1)
            if dist_label_sub.ndim == 2:
                dist_label_sub = dist_label_sub.squeeze(1)
            if goal_mask.ndim == 2:
                goal_mask = goal_mask.squeeze(1)
            if goal_mask_sub.ndim == 2:
                goal_mask_sub = goal_mask_sub.squeeze(1)
            if action_mask.ndim == 2:
                action_mask = action_mask.squeeze(1)
            if action_mask_sub.ndim == 2:
                action_mask_sub = action_mask_sub.squeeze(1)
            if goal_pos.ndim == 3:
                goal_pos = goal_pos.squeeze(1)
            if goal_pos_sub.ndim == 3:
                goal_pos_sub = goal_pos_sub.squeeze(1)
